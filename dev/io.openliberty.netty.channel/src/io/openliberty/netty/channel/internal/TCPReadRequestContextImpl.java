...
import io.openliberty.netty.channel.internal.TCPBaseRequestContext;
...
public abstract class TCPReadRequestContextImpl extends TCPBaseRequestContext implements TCPReadRequestContext, FFDCSelfIntrospectable {
    ...
    // TODO: this is a hack
    Queue<WsByteBuffer> queue = new LinkedList<WsByteBuffer>();
    void addBuffer(WsByteBuffer buf) {
        synchronized(queue) {
            System.out.println("addBuffer " + buf);

            queue.add(buf);
        }
    }

    Queue<WsByteBuffer> getQueuedBuffers() {
        return queue;
    }
    ...
    protected VirtualConnection readInternal(long numBytes, TCPReadCompletedCallback readCallback, boolean forceQueue, int timeout) {
        ...
        // TODO: this is a hack to get netty reads working with what existing channelf channels expect
        // queuing data up at this point is not OK
        synchronized(queue) {
            if (queue.peek() != null && queue.peek().hasRemaining() && getBuffer() != null && getBuffer().hasRemaining()) {
                int count = 0;
                while(getBuffer().hasRemaining() && queue.peek() != null) {
                    if (queue.peek().hasRemaining()) {
                        count++;
                        getBuffer().put(queue.peek().get());
                    } else {
                        queue.remove();
                    }
                }
                if (count > 0) {
                    System.out.println("readInternal used queue for read");
                    return getTCPConnLink().getVirtualConnection();
                }
            }
        }
        System.out.println("readInternal return null");
