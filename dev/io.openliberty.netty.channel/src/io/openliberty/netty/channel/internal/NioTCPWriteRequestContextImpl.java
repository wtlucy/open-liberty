...
public class NioTCPWriteRequestContextImpl extends TCPWriteRequestContextImpl {
    ...
    public long processSyncWriteRequest(long numBytes, int timeout) throws IOException {
        ...
        if (numBytes != 0) {
            ...
            if (this.channel != null) {
                for (WsByteBuffer wsbb : this.getBuffers()) {
                    numUserBytesWritten += wsbb.remaining();
                    this.channel.write(wsbb);
                }
                this.channel.flush();
            ...
            while (vc == null) {
                ...
                if (this.blockingIOError == null) {
                    vc = ((NettyNioTCPChannel) getTCPConnLink().getTCPChannel()).getWorkQueueManager().processWork(this, 1);
                } else {
                    break;
                }
            }
    ...
    public VirtualConnection processAsyncWriteRequest() {
        // TODO write to underlying Netty channel, assume it works immediately for now
        if (this.channel != null) {
            for (WsByteBuffer wsbb : this.getBuffers()) {
                this.channel.write(wsbb);
            }
        ...
        TCPConnLink conn = getTCPConnLink();
        VirtualConnection vc = conn.getVirtualConnection();
        return vc;
    }
