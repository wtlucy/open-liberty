 ...
 */
public class NettyChannelFactory implements ChannelFactory, CrossRegionSharable {

    private static final TraceComponent tc = Tr.register(NettyChannelFactory.class, TCPChannelMessageConstants.TCP_TRACE_NAME, TCPChannelMessageConstants.TCP_BUNDLE);

    private static final Class<?> appSideClass = TCPConnectionContext.class;

    private final Hashtable<String, ChannelTermination> terminationList = new Hashtable<String, ChannelTermination>();
...
//            final ClassLoader cl = NettyChannelFactory.class.getClassLoader();
//            if (cl != null) {
//                try {
...
//        }
    ...
    protected Channel createChannel(ChannelData channelData) throws ChannelException {
        ...
        // TODO WTL: ChannelFrameworkImpl is chfw-internal, skip for now 
        boolean asyncIOEnabled = false;
        if ((!newCC.isNIOOnly())
            && (commClass != null)
            && (asyncIOEnabled)) {
        ...
        if (!isOverrideClassUsed) {
            channel = new NettyNioTCPChannel();
            ChannelTermination ct = channel.setup(channelData, newCC, this);
            if (ct != null) {
    ...
    private static class TCPOutboundDefinition implements OutboundChannelDefinition {
        ...
        public Class<?> getOutboundFactory() {
            return NettyChannelFactory.class;
        }
