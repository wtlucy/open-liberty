...
import io.openliberty.netty.channel.internal.BindInfo;
import io.openliberty.netty.channel.internal.NewConnectionInitialReadCallback;
...
import io.openliberty.netty.channel.internal.TCPFactoryConfiguration;
...
import io.openliberty.netty.channel.internal.ValidateUtils;
...
//        this.serverSocket.bind(address, this.tcpChannel.getConfig().getListenBacklog());
        ...
        // TODO: setReuseAddress for netty
        ChannelFuture nettyChannelFuture = this.tcpChannel.nettyBootStrap.bind(address);
        try {
        ...
        System.out.println("NettyChannel available at: " + address);
    ...
    public synchronized void finishInitServerSocket() throws IOException, RetryableChannelException {
        ...
        while (currentTry <= portOpenRetries + 1) {
            ...
            if (bindError == null) {
                // listen port can be different than config port if configed port is '0'
//                listenPort = serverSocket.getLocalPort();
                InetAddress nettyAddress = ((InetSocketAddress) nettyChannel.localAddress()).getAddress();
                listenPort = ((InetSocketAddress) nettyChannel.localAddress()).getPort();

                String hostName = null;
                String IPvType = "IPv4";
                Tr.debug(tc, "serverSocket getInetAddress is: " + nettyAddress);
                Tr.debug(tc, "serverSocket getLocalSocketAddress is: " + nettyChannel.localAddress());
                Tr.debug(tc, "serverSocket getInetAddress hostname is: " + nettyAddress.getHostName());
                Tr.debug(tc, "serverSocket getInetAddress address is: " + nettyAddress.getHostAddress());
...
//                Tr.debug(tc, "serverSocket getInetAddress address is: " + serverSocket.getInetAddress().getHostAddress());
                ...
                if (nettyAddress instanceof Inet6Address) {
                    IPvType = "IPv6";
                }
...
//                }
                ...
                if (tcpChannel.config.getHostname() == null) {
                    ...
                    hostName = nettyAddress.getHostName() + "  (" + IPvType + ": "
                               + nettyAddress.getHostAddress() + ")";
                }
